...
<template>
  <PageHeader
    class="w-full flex flex-wrap h-16 bg-black bg-opacity-70 backdrop-blur-xl rounded-xl items-center justify-center sm:justify-around max-w-7xl p-2 sm:p-4 mx-auto mb-2"
  >
    <!-- existing HeaderLinks -->
  </PageHeader>

  <PageMain
    class="w-full flex flex-col h-full bg-black bg-opacity-70 backdrop-blur-xl rounded-xl items-center justify-start max-w-7xl mx-auto overflow-hidden"
  >
    <div class="w-full h-full p-2 sm:p-4 overflow-y-auto">
      <div v-if="loading" class="text-white text-center py-4">Loading assets...</div>
      ...
      <div v-else class="space-y-4">
        ...
        <div v-if="getTrendingAsset()" class="w-full mb-4">
          <h2 class="text-white text-xl sm:text-2xl font-bold mb-2 px-1">
            Trending Now on TradeBlazer
          </h2>
          <div
            class="bg-white bg-opacity-20 p-3 rounded-xl transform transition-all duration-300 hover:bg-opacity-30 hover:shadow-2xl cursor-pointer w-full"
          >
            <div
            ...
            >
              <div>
                ...
                <span class="text-gray-300 text-lg sm:text-xl mt-1 sm:mt-2 block">{{
                  getTrendingAsset()?.symbol
                }}</span>
                <span class="text-gray-400 uppercase text-base sm:text-lg mt-1 block">{{
              ...
              <div class="w-full sm:w-auto text-left sm:text-right mt-4 sm:mt-0">
                <span class="text-green-400 text-2xl sm:text-3xl font-bold block"
                  >${{ getTrendingAsset()?.price.toFixed(2) }}</span
                >
              </div>
            </div>
          </div>
        ...
        <div class="w-full">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 w-full">
            <div
              ...
              :key="asset.id"
              class="bg-white bg-opacity-10 p-3 rounded-lg transform transition-all duration-300 hover:bg-opacity-20 hover:shadow-xl cursor-pointer break-words min-w-0 w-full"
            >
              ...
              <div class="mt-2 space-y-1 sm:space-y-2">
                ...
                <span class="text-gray-400 block uppercase text-xs sm:text-sm">{{
                  asset.type
                }}</span>
                <span class="text-green-400 text-base sm:text-lg font-bold block"
                  >${{ asset.price.toFixed(2) }}</span
                >
              </div>
            </div>
          </div>
